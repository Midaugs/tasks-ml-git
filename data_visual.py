...
def plot_actual_vs_forecast_scatter(hist_db_path: str, forecast_db_path: str):

    with sqlite3.connect(hist_db_path) as conn:
        df_hist = pd.read_sql_query("SELECT * FROM observations", conn)
    ...
    with sqlite3.connect(forecast_db_path) as conn:
        df_forecast = pd.read_sql_query("SELECT * FROM Vilniusforecast", conn)
...
def interpolate_and_plot_5min_temperature(hist_db_path: str, forecast_db_path: str):
    ...
    with sqlite3.connect(forecast_db_path) as conn:
        df_forecast = pd.read_sql_query("SELECT forecastTimeUtc, airTemperature FROM Vilniusforecast", conn)
    df_forecast['forecastTimeUtc'] = pd.to_datetime(df_forecast['forecastTimeUtc'])
...
def calculate_yearly_average_temperature(db_path: str):
    ...
    return average_temp, first_time, last_time
...
def count_rainy_weekend_days(db_path: str) -> int:
    ...
    df['weekday'] = df['observationTimeUtc'].dt.weekday  
    weekends = df[df['weekday'].isin([5, 6])]
    rain_conditions = {'rain-showers', 'light-rain', 'rain'}
